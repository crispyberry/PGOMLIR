...
struct BranchOpLowering : public ConvertOpToLLVMPattern<cf::BranchOp> {
  ...
  matchAndRewrite(cf::BranchOp op, typename cf::BranchOp::Adaptor adaptor,
    ...
    if (failed(verifyMatchingValues(rewriter, adaptor.getDestOperands(),
      ...
      // auto llvmBrOp = rewriter.replaceOpWithNewOp<LLVM::BrOp>(
      //     op, adaptor.getOperands(), op->getSuccessors(), op->getAttrs());
      auto llvmBrOp = rewriter.create<LLVM::BrOp>(
          op->getLoc(), adaptor.getOperands(), op->getSuccessors());
      llvm::errs() << "SS!\n";
      if (auto cfBrOpAttr = (*op).getAttrOfType<StringAttr>("loop")) {
        llvm::errs() << "SSS!\n";
        Attribute attr = cfBrOpAttr.cast<Attribute>();
        (*llvmBrOp).setAttr("loop", attr);
      }
      rewriter.replaceOp(op, llvmBrOp->getResults());
      return success();
    }
  }
};
...
struct CondBranchOpLowering : public ConvertOpToLLVMPattern<cf::CondBranchOp> {
  ...
  matchAndRewrite(cf::CondBranchOp op,
    ...
    auto llvmCondBrOp =
        rewriter.create<LLVM::CondBrOp>(op->getLoc(), adaptor.getOperands(),
                                        op->getSuccessors(), op->getAttrs());
    if (auto cfCondBrOpAttr = (*op).getAttrOfType<StringAttr>("ifExpr")) {
      Attribute attr = cfCondBrOpAttr.cast<Attribute>();
      (*llvmCondBrOp).setAttr("ifExpr", attr);
    }
    rewriter.replaceOp(op, llvmCondBrOp->getResults());
    return success();
  }
};
...
void mlir::pgomlir::populateControlFlowToLLVMConversionPatterns(
...
struct CFToLLVMWithAttrPass
    : public CFToLLVMWithAttrBase<CFToLLVMWithAttrPass> {
  CFToLLVMWithAttrPass() = default;
  ...
  void runOnOperation() override {
    ...
    populateControlFlowToLLVMConversionPatterns(converter, patterns);
...
std::unique_ptr<Pass> mlir::pgomlir::createCFToLLVMWithAttrPass() {
  return std::make_unique<CFToLLVMWithAttrPass>();
}
